...
def write(
    ...
    Usage:
    * This tool will overwrite the file if it already exists unless the
      overwrite flag is set to False.
    * ALWAYS prefer editing existing files in the codebase. ONLY write new
      files when necessary.
    * NEVER proactively create documentation files or README files unless
      explicitly requested by the user.
    * Avoid using emojis in the file content unless asked by the user.
    * _CRITICAL_: DO NOT create empty files unless absolutely necessary. Even
      for files like __init__.py, include a comment with the file name.
    """
    file_exists = os.path.exists(file_path)

    if file_exists and not overwrite:
        return (
            f"Error: File {file_path} already exists. If you meant to edit the "
            "file use one of the available edit tools, if this was intentional, "
