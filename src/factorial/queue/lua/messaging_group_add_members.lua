--[[
-- Add members to an existing team-scoped messaging group atomically.
--
-- Validates sender membership, validates all candidate members against team
-- scope, then updates group membership and reverse indexes in one transaction.
--
-- State transitions:
-- - group exists -> members added (idempotent)
...
local added_member_task_ids = {}
for _, member_task_id in ipairs(unique_member_task_ids) do
    local added = redis.call("SADD", group_members_key, member_task_id)
    redis.call("SADD", groups_by_task_key(member_task_id), group_name)
    redis.call("SADD", team_tasks_key, member_task_id)
    if added == 1 then
        table.insert(added_member_task_ids, member_task_id)
    end
end

return { "updated", cjson.encode(added_member_task_ids) }
