...
_REGISTERED_SUBSCRIPTIONS: set[tuple[int, str, str]] = set()
...
def create_worker(
    ...
    *,
    ...
    if registration_key not in _REGISTERED_SUBSCRIPTIONS:

        @subscribe(topic=settings.dispatch_topic, consumer=settings.dispatch_consumer)
        async def _queue_worker(message: Any, metadata: dict[str, Any]) -> None:
        ...
        _REGISTERED_SUBSCRIPTIONS.add(registration_key)
    ...
    *,
    ...
    if envelope.kind == "maintenance_tick":
        ...
        if (
            ...
            await orchestrator.wake_maintenance(reason="maintenance_continuation")
